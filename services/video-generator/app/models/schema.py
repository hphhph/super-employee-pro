...
class VideoParams(BaseModel):
    ...
    video_clip_duration: int = Field(default=10, ge=1, le=120)
    ...
    watermark_image: Optional[str] = ""  # 水印图片文件名，位于 resource/watermarks/ 目录
    watermark_position: Optional[str] = "bottom-right"  # top-left/top-right/bottom-left/bottom-right/center
    watermark_opacity: Optional[float] = 0.5  # 0.0~1.0
    watermark_scale: Optional[float] = 0.15  # 相对视频宽度的比例

    # Webhook 回调通知
    webhook_url: Optional[str] = ""
    webhook_events: Optional[str] = "complete,failed"  # 触发回调的事件类型
...
class TaskStatusData(BaseModel):
    ...
    # 国内平台（抖音/快手/视频号/小红书）发布状态，独立于 upload_post。
    china_cross_post_state: Optional[
        Literal["pending", "processing", "complete", "failed"]
    ] = None
    china_cross_post_results: Optional[List[dict[str, Any]]] = None
    china_cross_post_error: Optional[str] = None
