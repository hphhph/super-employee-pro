...
def _resolve_watermark_path(watermark_image: str) -> str:
    """解析水印图片路径，限制在项目 resource 目录内。"""
    if not watermark_image:
        return ""

    # 允许直接传入绝对路径（仅限 resource 目录下）或相对文件名
    if path.isabs(watermark_image):
        project_root = path.realpath(utils.root_dir())
        real_path = path.realpath(watermark_image)
        try:
            if path.commonpath([project_root, real_path]) != project_root:
                logger.warning(
                    f"reject watermark path outside project dir: {watermark_image}"
                )
                return ""
        except ValueError:
            return ""
        return real_path if path.isfile(real_path) else ""

    # 相对路径：查找 resource/watermarks/ 和 resource/ 目录
    search_dirs = [
        utils.storage_dir("watermarks", create=False),
        path.join(utils.root_dir(), "resource"),
    ]
    for search_dir in search_dirs:
        candidate = path.join(search_dir, watermark_image)
...
def _create_watermark_clip(
    ...
    参数:
      ...
      opacity: 0.0~1.0
      scale: 水印宽度相对视频宽度的比例
      duration: 水印显示时长（默认与视频同长）
    """
    try:
        wm_clip, _ = _open_image_clip_with_fallback(watermark_path)
    except Exception as exc:
    ...
    wm_clip = wm_clip.resized(width=target_w)

    # 设置透明度
    safe_opacity = max(0.0, min(1.0, float(opacity)))
    if safe_opacity < 1.0:
        if wm_clip.mask is None:
            wm_clip = wm_clip.with_mask()
        wm_clip = wm_clip.with_effects([afx.MultiplyVolume(safe_opacity)])

    # 实际上 ImageClip 的 mask 控制透明度更可靠，这里用 mask 调整
    if safe_opacity < 1.0 and wm_clip.mask is not None:
        original_opacity_factor = safe_opacity
        # MoviePy 的 mask 是 0~1 的亮度值，直接缩放 mask 即可调整透明度
        wm_clip = wm_clip.masked(
            lambda mf: mf * original_opacity_factor
        )
    ...
    # 计算位置
    margin = max(10, int(video_width * 0.02))  # 边距约 2% 视频宽度
    wm_w, wm_h = wm_clip.size

    pos_map = {
        "top-left": (margin, margin),
        "top-right": (video_width - wm_w - margin, margin),
        "bottom-left": (margin, video_height - wm_h - margin),
        "bottom-right": (video_width - wm_w - margin, video_height - wm_h - margin),
        "center": (
            int((video_width - wm_w) / 2),
            int((video_height - wm_h) / 2),
        ),
    }
...
def generate_video(
    ...
    with ExitStack() as clip_stack:
        ...
        # 水印叠加：在字幕之后、音频混合之前添加，确保水印始终位于画面最上层。
        if params.watermark_enabled and params.watermark_image:
            wm_path = _resolve_watermark_path(params.watermark_image)
            if wm_path:
                wm_clip = _create_watermark_clip(
                    watermark_path=wm_path,
                    video_width=video_width,
                    video_height=video_height,
                    position=params.watermark_position or "bottom-right",
                    opacity=params.watermark_opacity or 0.5,
                    scale=params.watermark_scale or 0.15,
                    duration=video_clip.duration,
                )
                if wm_clip is not None:
                    video_clip = CompositeVideoClip([video_clip, wm_clip])
                    clip_stack.callback(video_clip.close)
                    logger.info(f"watermark applied: {params.watermark_image}")
            else:
                logger.warning(
                    f"watermark enabled but image not found: {params.watermark_image}"
                )
