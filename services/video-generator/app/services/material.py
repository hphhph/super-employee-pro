...
def search_videos_pixabay(
    ...
    try:
        ...
        for v in videos:
            ...
            # loop through each url to determine the best quality
            # 在满足目标分辨率的档位中选"最小"的：Pixabay 部分素材的 large
            # 档高达 4K（3840x2160），文件几十 MB，下载极慢且 1080p 成片用
            # 不上；选最小满足档可显著减少下载量，对成片画质无损失。
            best_video = None
            best_video_type = None
            best_area = None
...
def download_videos(
    ...
    elif source in ("yingshiju", "aigei", "jimeng"):
        from app.services import material_cn
        cn_provider = material_cn.CN_MATERIAL_PROVIDERS.get(source)
        if cn_provider is None:
            logger.error(f"unsupported cn material source: {source}")
            return []
        provider = source
        remote_search_videos = cn_provider
    ...
    def _download_one(item: MaterialInfo):
        """并发下载单个素材。返回 (saved_video_path, item) 或 None。"""
    ...
    video_paths = []
    material_sources: list[dict[str, Any]] = []
    # 并发下载：6 个 worker 同时拉取素材，显著缩短下载总时长。
    # random 拼接模式下素材顺序无要求；script_order 模式走独立的保序函数，不受影响。
    with ThreadPoolExecutor(max_workers=6, thread_name_prefix="material-dl") as pool:
        futures = [pool.submit(_download_one, item) for item in valid_video_items]
        for fut in as_completed(futures):
            result = fut.result()
            if result is None:
                continue
            saved_video_path, item = result
