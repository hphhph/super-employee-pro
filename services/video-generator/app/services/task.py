...
_china_cross_post_max_pending_tasks = max(
    1,
    int(config.app.get("china_publish_max_pending_tasks", 10)),
)
_china_cross_post_slots = threading.BoundedSemaphore(
    _china_cross_post_max_pending_tasks
)
_cross_post_registry_lock = threading.RLock()
_cross_post_futures: dict[str, Future] = {}
_china_cross_post_futures: dict[str, Future] = {}
...
_ACTIVE_CHINA_CROSS_POST_STATES = {
    const.CHINA_CROSS_POST_STATE_PENDING,
    const.CHINA_CROSS_POST_STATE_PROCESSING,
}
...
def is_task_busy(task: dict | None) -> bool:
    ...
    return (
        ...
        or task.get("china_cross_post_state") in _ACTIVE_CHINA_CROSS_POST_STATES
    )
...
def _register_china_cross_post_future(task_id: str, future: Future) -> None:
    with _cross_post_registry_lock:
        _china_cross_post_futures[task_id] = future


def _unregister_china_cross_post_future(
    task_id: str, future: Future | None = None
) -> None:
    with _cross_post_registry_lock:
        current = _china_cross_post_futures.get(task_id)
        if current is None or (future is not None and current is not future):
            return
        _china_cross_post_futures.pop(task_id, None)


def _is_china_cross_post_active_in_process(task_id: str) -> bool:
    with _cross_post_registry_lock:
        future = _china_cross_post_futures.get(task_id)
        return future is not None and not future.done()
...
def recover_interrupted_cross_posts(page_size: int = 100) -> int | None:
    ...
    while True:
        ...
        for task in tasks:
            ...
            # 国际平台发布恢复
            if (
                task.get("cross_post_state") in _ACTIVE_CROSS_POST_STATES
                and not _is_cross_post_active_in_process(task_id)
            ...
            # 国内平台发布恢复
            if (
                task.get("china_cross_post_state") in _ACTIVE_CHINA_CROSS_POST_STATES
                and not _is_china_cross_post_active_in_process(task_id)
            ):
                china_updated = _patch_cross_post_state(
                    task_id,
                    china_cross_post_state=const.CHINA_CROSS_POST_STATE_FAILED,
                    china_cross_post_error=_INTERRUPTED_CROSS_POST_ERROR,
                )
                if china_updated is True:
                    recovered += 1
...
def _run_china_cross_post(
    ...
    try:
        state_updated = _patch_cross_post_state(
            task_id,
            china_cross_post_state=const.CHINA_CROSS_POST_STATE_PROCESSING,
            china_cross_post_error=None,
        )
        if state_updated is not True:
            if state_updated is False:
                logger.warning(
                    f"skip china cross-post for missing task: {task_id}"
                )
            else:
                _patch_cross_post_state(
                    task_id,
                    china_cross_post_state=const.CHINA_CROSS_POST_STATE_FAILED,
                    china_cross_post_error=(
                        "failed to persist china cross-post processing state"
                    ),
                )
            return

        logger.info(
            f"china cross-post started, task_id: {task_id}, "
        ...
        for video_path in video_paths:
            result = china_publish.cross_post_to_china(
                video_path=video_path,
                title=title,
        ...
        for r in results:
            ...
            failures += r.get("failed_count", 0)

        if failures:
            error_messages = [
                str(r.get("error") or r.get("platform") + " failed")
                for r in all_platform_results
                if not r.get("success")
            ]
            china_cross_post_state = const.CHINA_CROSS_POST_STATE_FAILED
            china_cross_post_error = "; ".join(error_messages[:5])
            logger.warning(
                f"china cross-post completed with failures, task_id: {task_id}, "
                f"failed: {failures}, total: {len(all_platform_results)}"
            )
        else:
            china_cross_post_state = const.CHINA_CROSS_POST_STATE_COMPLETE
            china_cross_post_error = None
            logger.success(
                f"china cross-post completed, task_id: {task_id}, "
                f"videos: {len(results)}"
            )

        _patch_cross_post_state(
            task_id,
            china_cross_post_state=china_cross_post_state,
            china_cross_post_results=all_platform_results,
            china_cross_post_error=china_cross_post_error,
        )
    except Exception as exc:
        logger.exception(
            f"china cross-post failed, task_id: {task_id}, error: {exc}"
        )
        _patch_cross_post_state(
            task_id,
            china_cross_post_state=const.CHINA_CROSS_POST_STATE_FAILED,
            china_cross_post_error=str(exc),
            china_cross_post_results=results or None,
        )


def _run_china_cross_post_with_slot(*args) -> None:
    try:
        _run_china_cross_post(*args)
    except Exception as exc:
        task_id = str(args[0]) if args else "unknown"
        logger.exception(
            f"china cross-post worker crashed, task_id: {task_id}, error: {exc}"
        )
        if args:
            _patch_cross_post_state(
                str(args[0]),
        ...
        _china_cross_post_slots.release()


def _finalize_china_cross_post_future(task_id: str, future: Future) -> None:
    _unregister_china_cross_post_future(task_id, future)
    try:
        error = future.exception()
    except CancelledError:
        logger.warning(
            f"china cross-post future was cancelled, task_id: {task_id}"
        )
        _china_cross_post_slots.release()
        _patch_cross_post_state(
            task_id,
            china_cross_post_state=const.CHINA_CROSS_POST_STATE_FAILED,
            china_cross_post_error="china cross-post job was cancelled",
        )
        return
    except Exception as exc:
        logger.exception(
            f"failed to inspect china cross-post future, task_id: {task_id}, "
            f"error: {exc}"
        )
        return
    ...
    if error is not None:
        logger.error(
            f"china cross-post future failed, task_id: {task_id}, "
            f"error: {type(error).__name__}: {error}"
        )
    ...
    try:
        ...
        return
    if task and task.get("china_cross_post_state") in _ACTIVE_CHINA_CROSS_POST_STATES:
        _patch_cross_post_state(
            task_id,
            china_cross_post_state=const.CHINA_CROSS_POST_STATE_FAILED,
            china_cross_post_error=(
                task.get("china_cross_post_error")
                or "china cross-post worker ended without terminal state"
            ),
        )


def _schedule_china_cross_post(
    task_id: str,
    video_paths: list[str],
    params: VideoParams,
    video_script: str,
    ...
    """提交国内平台后台发布任务。"""
    if not _china_cross_post_slots.acquire(blocking=False):
        error = "china cross-post queue is full; publishing was skipped"
        logger.warning(
            f"skip china cross-post because queue is full, task_id: {task_id}"
        )
        _patch_cross_post_state(
            task_id,
            china_cross_post_state=const.CHINA_CROSS_POST_STATE_FAILED,
            china_cross_post_error=error,
        )
        return error

    try:
        future = _cross_post_executor.submit(
            _run_china_cross_post_with_slot,
            task_id,
            tuple(video_paths),
            params.video_subject or "",
            video_script,
        ...
        _register_china_cross_post_future(task_id, future)
        future.add_done_callback(
            partial(_finalize_china_cross_post_future, task_id)
        )
    except RuntimeError as exc:
        _unregister_china_cross_post_future(task_id)
        _china_cross_post_slots.release()
        logger.exception(
            f"failed to schedule china cross-post, task_id: {task_id}, error: {exc}"
        )
        _patch_cross_post_state(
            task_id,
            china_cross_post_state=const.CHINA_CROSS_POST_STATE_FAILED,
            china_cross_post_error=f"failed to schedule china cross-post: {exc}",
        )
        return f"failed to schedule china cross-post: {exc}"
...
def _run_pipeline(
    ...
    # 国内平台发布（抖音/快手/视频号/小红书）独立于 upload_post，
    # 复用同一套异步调度框架但使用独立的状态字段和队列容量。
    china_publish_service = china_publish.china_publish_service
    china_cross_post_enabled = (
        china_publish_service.is_configured()
        and china_publish_service.auto_upload
    )
    china_platforms = (
        list(china_publish_service.platforms)
        if china_cross_post_enabled
        else []
    )
    should_china_cross_post = china_cross_post_enabled and bool(china_platforms)
    if china_cross_post_enabled and not china_platforms:
        logger.warning(
            f"skip china cross-post because no platforms are configured, "
            f"task_id: {task_id}"
        )
    china_cross_post_state = (
        const.CHINA_CROSS_POST_STATE_PENDING if should_china_cross_post else None
    )
    ...
    if should_china_cross_post:
        china_scheduling_error = _schedule_china_cross_post(
            task_id=task_id,
            video_paths=final_video_paths,
            params=params,
            video_script=video_script,
        ...
        if china_scheduling_error:
            kwargs["china_cross_post_state"] = (
                const.CHINA_CROSS_POST_STATE_FAILED
            )
            kwargs["china_cross_post_error"] = china_scheduling_error
...
def start(
    ...
    # 任务开始时发送 webhook 通知（如果配置了 pending 事件）
    try:
        pending_task_data = {"state": const.TASK_STATE_PROCESSING, "progress": 0}
    ...
    try:
        ...
        try:
            if isinstance(result, dict):
                ...
                if result_state == const.TASK_STATE_COMPLETE:
                    webhook_service.notify(
                        task_id=task_id,
                        event="complete",
                        task_data=result,
                ...
                elif result_state == const.TASK_STATE_FAILED:
                    webhook_service.notify(
                        task_id=task_id,
                        event="failed",
                        task_data=result,
