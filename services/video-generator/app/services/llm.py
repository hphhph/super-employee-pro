...
def _generate_response(prompt: str, app_config=None) -> str:
    try:
        ...
        # 页面配置（系统设置 → API密钥配置）优先，后端不可用时回退本地 config.toml
        from app.services import remote_keys

        remote = remote_keys.get_remote(llm_provider) or {}
        api_key = (
            remote.get("apiKey")
            or runtime_app_config.get(provider.config_key("api_key"), "")
        )
        configured_model = (
            remote.get("model")
            or runtime_app_config.get(provider.config_key("model_name"), "")
        )
        ...
        configured_base_url = (
            remote.get("baseUrl")
            or runtime_app_config.get(provider.config_key("base_url"), "")
        )
